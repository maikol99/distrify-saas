  ...
  actions: {
    async fetchInputs() {
      ...
      try {
        const response = await api.get(
          `/inputs/get/all-inputs?shopId=${shopId}&page=${this.pagination.page}&limit=${this.pagination.limit}`
        );
    ...
    async filterInputs() {
      ...
      try {
        ...
        const response = await api.get(
          `/inputs/get/inputs-filtered?${params.toString()}`
        );
        const data = response.data;
        ...
        if (data.success) {
          ...
          this.toast = {
            showing: true,
            message: data.message || "No se encontraron ingresos",
            state: "information",
          };
        }
      } catch (error) {
