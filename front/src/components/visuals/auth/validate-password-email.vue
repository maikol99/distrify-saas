    ...
    <div class="validate-card">
      ...
      <div class="header-section">
        <div class="icon-wrapper" :class="{ success: validationSuccess, error: !loading && !validationSuccess && errorMessage }">
          ...
          <span class="material-symbols-outlined" v-else-if="validationSuccess">lock_reset</span>
          <span class="material-symbols-outlined" v-else>error_outline</span>
        </div>
        <h1 class="title">
          <template v-if="loading">Validando enlace...</template>
          <template v-else-if="validationSuccess">¡Enlace válido!</template>
          <template v-else>Enlace inválido</template>
        </h1>
        <p class="subtitle">
          <template v-if="loading">Verificando tu código de recuperación</template>
          <template v-else-if="validationSuccess">Podés continuar para crear tu nueva contraseña</template>
      ...
      <div class="content-section">
        ...
        <div v-if="!loading && validationSuccess" class="result-box result-success">
          ...
          <p>El enlace es válido. Presioná "Continuar" para restablecer tu contraseña.</p>
        </div>
        ...
        <div v-if="!loading && !validationSuccess && errorMessage" class="result-box result-error">
          ...
          <p>El enlace expiró o no es válido. Solicitá uno nuevo desde la página de login.</p>
        </div>
      </div>

      <div class="actions-section" v-if="!loading">
        <button v-if="validationSuccess" @click="redirectToReset" class="btn-primary">
          Continuar
        </button>
...
export default {
  ...
  methods: {
    async validateToken() {
      ...
      try {
        ...
        const response = await api.post(
          `/auth/post/verify-password-reset-code?resetCode=${encodeURIComponent(token)}`
        );
        ...
        if (data.success) {
          ...
          this.showToast("Token validado con éxito.", "success");
        } else {
          this.validationSuccess = false;
    ...
    redirectToReset() {
      this.$router.push("/recuperar-contrasena" + `/${this.$route.params.token}`);
    },
  },
