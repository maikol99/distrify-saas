<template>
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] md:p-4 backdrop-blur-sm" @click.self="closeModal">
    <div class="bg-white w-screen h-screen md:w-[600px] md:h-auto md:max-h-[90vh] md:rounded-3xl max-w-full max-h-full overflow-y-auto flex flex-col font-sans shadow-2xl">
      ...
      <div class="flex justify-between items-center p-5 border-b border-orange-100 bg-orange-50 md:rounded-t-3xl text-orange-600">
        ...
        <button @click="closeModal" class="text-orange-400 hover:text-red-500 transition-colors bg-transparent border-none cursor-pointer flex justify-center items-center">
      ...
      <form @submit.prevent="submitPayment">
        <div class="form-group">
          <label for="clientSearch">Proveedor</label>
          <div class="search-container">
            <input
              type="text"
              id="clientSearch"
              v-model="searchQuery"
              @input="supplierDebouncedSearch"
        ...
        <div class="form-group">
          ...
          <select name="" id="" v-model="paymentMethod">
        ...
        <div class="form-group">
          <label for="amount">Método de pago</label>
        ...
        <div class="p-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50 md:rounded-b-3xl mt-auto shadow-inner">
          <button type="button" @click="closeModal" class="py-2 px-5 bg-white border border-gray-200 text-gray-600 rounded-xl font-semibold cursor-pointer transition-colors hover:bg-gray-100 hover:text-gray-800 shadow-sm w-full md:w-auto" :disabled="loading">
            Cancelar
          </button>
          <button type="submit" class="py-2 px-5 bg-orange-500 border-none text-white rounded-xl font-semibold cursor-pointer transition-colors hover:bg-orange-600 shadow-sm flex items-center justify-center disabled:bg-gray-400 disabled:cursor-not-allowed w-full md:w-auto" :disabled="loading">
    ...
    async searchSuppliers() {
      ...
      try {
        ...
        const response = await api.get(
          `/suppliers/get/search-supplier/by-name?name=${this.searchQuery}&shopId=${shopId}`
        );
        if (response.data.success) {
          this.filteredSuppliers = response.data.suppliers;
          this.showResults = true;
        } else {
          console.error("Error al buscar Proveedores:", response.data.message);
          this.filteredSuppliers = [];
    ...
    async submitPayment() {
      ...
      try {
        const response = await api.post(
          `/supplier-payments/post/create-payment`, // Usar el endpoint de creación de pagos
          {
