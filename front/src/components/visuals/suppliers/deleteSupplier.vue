  ...
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] md:p-4 backdrop-blur-sm" @click="close">
    <div class="bg-white w-screen h-screen md:w-[500px] md:h-auto md:max-h-[90vh] md:rounded-3xl max-w-full max-h-full overflow-y-auto flex flex-col font-sans shadow-2xl" @click.stop>
      ...
      <div class="modal-body p-5 flex-1 overflow-y-auto">
        <div class="confirmation-content flex items-start gap-4 mb-4 flex-col md:flex-row text-center md:text-left">
          ...
          <div class="confirmation-text">
            <p><strong>¿Estás seguro de que quieres eliminar este proveedor?</strong></p>
            <p>Esta acción no se puede deshacer. El proveedor será eliminado permanentemente del sistema.</p>
          </div>
        </div>
      ...
      <div class="p-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50 md:rounded-b-3xl mt-auto shadow-inner">
        ...
        <button @click="deleteSupplier" class="py-2 px-5 bg-red-600 border-none text-white rounded-xl font-semibold cursor-pointer transition-colors hover:bg-red-700 shadow-sm flex items-center justify-center disabled:bg-red-400 disabled:cursor-not-allowed w-full md:w-auto" :disabled="loading">
    ...
    async deleteSupplier() {
      try {
        ...
        const response = await api.delete(
          `/suppliers/delete/delete-supplier/${this.supplierId}`
        );
        ...
        if (data.success) {
          ...
          this.error = data.message || "Error al eliminar el proveedor";
        }
      } catch (error) {
        console.log(error);
