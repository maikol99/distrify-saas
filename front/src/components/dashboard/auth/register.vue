<template>
  ...
  >
    <!-- Left: Form Panel -->
    <div class="flex-1 flex items-center justify-center bg-slate-50 dark:bg-slate-950 relative overflow-hidden px-6 py-2">
      <!-- Background Decor -->
      <div
      ...
      >
        ...
        <form @submit.prevent="register" class="space-y-2.5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <!-- Username -->
            <div class="space-y-1 group animate-slide-up" style="animation-delay: 100ms">
              <label class="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Usuario</label>
              <div class="relative">
            ...
            <!-- Phone -->
            <div class="space-y-1 group animate-slide-up" style="animation-delay: 150ms">
              <label class="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Teléfono</label>
              <div class="relative">
                <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors text-[18px]">call</span>
                <input
                  v-model="phone"
                  type="text"
                  required
          ...
          <!-- Email -->
          <div class="space-y-1 group animate-slide-up" style="animation-delay: 200ms">
            <label class="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Email</label>
            <div class="relative">
              <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors text-[18px]">mail</span>
              <input
                v-model="email"
                type="email"
                required
                placeholder="hola@distrify.com"
                class="w-full pl-10 pr-3 py-2 bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white text-sm"
              />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <!-- Password -->
            <div class="space-y-1 group animate-slide-up" style="animation-delay: 250ms">
              <label class="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Contraseña</label>
              <div class="relative">
                <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors text-[18px]">lock</span>
                <input
              ...
              <div v-if="password" class="px-1">
                <div class="h-0.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div class="h-full transition-all duration-500" :class="{ 'bg-red-500': passwordStrength === 'weak', 'bg-orange-500': passwordStrength === 'medium', 'bg-green-500': passwordStrength === 'strong' }" :style="{ width: getStrengthWidth() }"></div>
                </div>
                <span class="text-[10px] font-bold uppercase tracking-wider mt-0.5 block" :class="{ 'text-red-500': passwordStrength === 'weak', 'text-orange-500': passwordStrength === 'medium', 'text-green-500': passwordStrength === 'strong' }">{{ getStrengthText() }}</span>
              </div>
            </div>
            <!-- Confirm Password -->
            <div class="space-y-1 group animate-slide-up" style="animation-delay: 300ms">
              <label class="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Confirmar</label>
              <div class="relative">
                <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors text-[18px]">verified_user</span>
                <input
              ...
              <div v-if="confirmPassword" class="flex items-center gap-1 mt-0.5 ml-1">
                <span class="material-symbols-outlined text-[13px]" :class="passwordsMatch ? 'text-green-500' : 'text-red-500'">{{ passwordsMatch ? "check_circle" : "cancel" }}</span>
                <span class="text-[10px] font-bold uppercase tracking-wider" :class="passwordsMatch ? 'text-green-500' : 'text-red-500'">{{ passwordsMatch ? "Coinciden" : "No coinciden" }}</span>
              </div>
            </div>
          </div>
        ...
        <div class="mt-3">
          <div class="relative flex items-center justify-center py-2.5">
            ...
            <span class="bg-slate-50 dark:bg-slate-950 px-3 text-xs font-bold text-slate-400 absolute">O CON GOOGLE</span>
          </div>
      ...
      <div class="relative z-10 text-center max-w-md">
        ...
        <div class="flex flex-col gap-4 text-left">
          <div class="flex items-center gap-4">
            ...
            <div>
              <p class="text-white font-bold text-sm">Gestión de tienda completa</p>
              <p class="text-slate-400 text-xs">Productos, stock, precios y más.</p>
            </div>
          </div>
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-xl bg-primary/20 flex items-center justify-center flex-shrink-0">
            ...
            <div>
              <p class="text-white font-bold text-sm">Punto de venta integrado</p>
              <p class="text-slate-400 text-xs">Vendé rápido con caja y turnos.</p>
            </div>
          </div>
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-xl bg-primary/20 flex items-center justify-center flex-shrink-0">
            ...
            <div>
              <p class="text-white font-bold text-sm">Reportes en tiempo real</p>
              <p class="text-slate-400 text-xs">Tomá decisiones basadas en datos.</p>
            </div>
          </div>
        </div>
      </div>

      <p class="absolute bottom-6 text-slate-600 text-xs z-10">Distrify Analytics</p>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async registerWithGoogle() {
      window.open("https://distrify.com.ar/registro-google", "_blank");
    },
    async register() {
      try {
        this.loading = true;
