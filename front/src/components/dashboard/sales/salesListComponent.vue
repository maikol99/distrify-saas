...
export default {
  ...
  methods: {
    ...
    async emitirTicket() {
      ...
      console.log("Imprimiendo desde component ");
      ...
      const selectedSale = this.salesListStore.selectedSale;

      // Validación de datos mínimos requeridos
      if (!selectedSale._id && !selectedSale.createdAt) {
        console.warn("Venta sin datos mínimos requeridos");
        alert(
          "No se puede generar el ticket: faltan datos esenciales de la venta.",
        );
        return false;
      }
      ...
      try {
        ...
        const productDetails = selectedSale.productDetails || [];

        // Calcular altura estimada del contenido
        const baseHeight = 120; // Encabezado + aviso fiscal + pie de página
        const itemHeight = productDetails.length * 10; // ~10mm por item
        const variantHeight = productDetails.reduce((acc, item) => {
          if (
            item.variants &&
            Array.isArray(item.variants) &&
            item.variants.length > 0
          ) {
            return acc + item.variants.length * 3;
          }
          return acc;
        }, 0);
        const summaryHeight = 60; // Resumen financiero
        ...
        const isCancelled = selectedSale.status === "Cancelado";
        ...
        doc.setFontSize(11);
        doc.setFont("helvetica", "bold");
        doc.text("COMPROBANTE DE VENTA", pageWidth / 2, yPos, {
          align: "center",
        });
        yPos += 6;
        ...
        // Validación y generación del número de ticket
        const ticketNumber = `VT-${
          selectedSale._id?.slice(-8) || Date.now().toString().slice(-8)
        }`;
        ...
        try {
          ...
          console.warn("Error al parsear fecha de venta, usando fecha actual");
          saleDate = new Date();
        }

        // Información del ticket con validaciones
        const ticketInfo = [
          `Ticket No: ${ticketNumber}`,
          `Fecha venta: ${saleDate.toLocaleDateString("es-ES")}`,
          `Hora venta: ${saleDate.toLocaleTimeString("es-ES")}`,
          `Cliente: ${
            selectedSale.clientName ||
            selectedSale.clientId?.name ||
            "CONSUMIDOR FINAL"
          }`,
        ...
        if (selectedSale.listName) {
          ticketInfo.push(`Lista de precios: ${selectedSale.listName}`);
        }
        ...
        if (productDetails.length === 0) {
          ...
          productDetails.forEach((item, index) => {
            ...
            const productName = item.productName || item.productId?.name || "Producto sin nombre";
            ...
            // Nombre del producto con validación de longitud
            doc.setFont("helvetica", "bold");
            const truncatedName =
            ...
            // Mostrar variantes si existen y son válidas
            if (
              item.variants &&
              Array.isArray(item.variants) &&
              item.variants.length > 0
        ...
        // Cálculo total de items con validación
        ...
        // Recargo con validación
        const surcharge = selectedSale.surcharge || 0;
        if (surcharge > 0) {
        ...
        // === INFORMACIÓN DE PAGO DETALLADA CON VALIDACIONES ===
        doc.setFontSize(8);
        doc.setFont("helvetica", "bold");
        ...
        const paymentMethods = selectedSale.paymentMethods;
        if (
          paymentMethods &&
          ...
          paymentMethods.forEach((method) => {
            if (method && method.method) {
              ...
              doc.text(
                `${method.method}: $${amount.toFixed(2)}`,
                margin + 2,
                yPos,
              );
              yPos += 3;
          ...
          const paymentMethod = selectedSale.paymentMethod || "Efectivo";
        ...
        if (isCancelled) {
          ...
          doc.text("VENTA CANCELADA", pageWidth / 2, yPos, { align: "center" });
          doc.setTextColor(0, 0, 0);
        } else {
        ...
        yPos += 8; // Espacio final

        // Ajustar altura del documento al contenido real
        const finalHeight = yPos + 5; // Añadir pequeño margen final
